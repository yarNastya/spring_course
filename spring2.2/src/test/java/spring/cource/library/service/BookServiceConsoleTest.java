...
class BookServiceConsoleTest {
    ...
    void shouldGetAllBooks() {
        doReturn(List.of(new Book().setId(25), new Book().setId(26))).when(bookRepository).getAllBooks();
    ...
    void shouldGetBookById() {
        ...
        doReturn(Optional.of(expectedBook)).when(bookRepository).getById(EXPECTED_ID);
    ...
    void shouldAddNewBookSucessfully() {
        ...
        doReturn(Optional.of(EXPECTED_ID)).when(bookRepository).insert(any());
        doReturn(Optional.of(author)).when(authorRepository).getById(1L);
        doReturn(Optional.of(genre)).when(genreRepository).getById(11L);
    ...
    void shouldNotAddNewBook() {
        Author author = new Author().setId(1L);
        Genre genre = new Genre().setId(11);
        doReturn(Optional.empty()).when(bookRepository).insert(any());
        doReturn(Optional.of(author)).when(authorRepository).getById(1L);
        doReturn(Optional.of(genre)).when(genreRepository).getById(11L);
        String result = sut.insertBook("Test", 1L, 11L, 1777);
    ...
    void shouldUpdateBookSucessfully() {
        ...
        doReturn(Optional.of(book)).when(bookRepository).getById(EXPECTED_ID);
        doReturn(Optional.of(EXPECTED_ID)).when(bookRepository).update(any());
        doReturn(Optional.of(author)).when(authorRepository).getById(1L);
        doReturn(Optional.of(genre)).when(genreRepository).getById(11L);
