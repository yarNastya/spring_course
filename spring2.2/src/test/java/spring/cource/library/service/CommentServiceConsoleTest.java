...
class CommentServiceConsoleTest {
    ...
    void shouldInsertComment() {
        doReturn(EXPECTED_ID).when(commentRepository).insert(any());
        doReturn(Optional.of(new Book().setId(1L))).when(bookRepository).getById(1L);
    ...
    void updateComment() {
        doReturn(Optional.of(new Comment())).when(commentRepository).getById(EXPECTED_ID);
        doReturn(EXPECTED_ID).when(commentRepository).update(any());
